...
초기 실험 대상은 Tesla(`TSLA`)이지만, `yfinance`에서 조회 가능한 다른 미국 주식 티커와 한국 주식 티커도 함께 분석할 수 있습니다. 한국 코스피 종목은 `034020.KS`, 코스닥 종목은 `123456.KQ`처럼 Yahoo Finance 형식의 접미사를 붙여 입력합니다. 이 앱은 투자 판단용이 아니라 Prophet의 동작 방식, 이벤트 반영, 이상값 탐지, 백테스트 흐름을 학습하고 검증하기 위한 개인 프로젝트입니다.
...
한국 주식은 Yahoo Finance 티커 형식을 사용합니다.

- 코스피: `034020.KS`처럼 종목 코드 뒤에 `.KS`를 붙입니다.
- 코스닥: `123456.KQ`처럼 종목 코드 뒤에 `.KQ`를 붙입니다.
- 두산에너빌리티는 `034020.KS`로 프리셋에 포함되어 있습니다.
...
예측 날짜와 저장 데이터 커버리지 계산에 사용할 거래일 정책입니다.

- `Auto by ticker`: `.KS`, `.KQ` 티커는 한국장 거래일, 그 외 티커는 미국장 거래일을 사용합니다.
- `US stock trading days`: 주말과 NYSE 휴장일을 제외합니다.
- `Korea stock trading days`: 주말, 한국 공휴일, 근로자의 날, 연말 휴장일을 제외합니다.
- `Daily calendar days`: 모든 달력 날짜를 사용합니다.
